...
export function AgencyPage() {
  ...
  return <main className={loaded ? "site is-ready" : "site"}>
    ...
    <div className={`cursor ${cursor.label ? "cursor-active" : ""}`} style={{ transform: `translate3d(${cursor.x}px, ${cursor.y}px, 0)` }} aria-hidden="true">{cursor.label}</div>
    <header className="nav"><a className="wordmark" href="#top" onClick={closeMenu}>THESIS<span>®</span></a><nav className={menuOpen ? "open" : ""} aria-label="Primary navigation"><a href="#work" onClick={closeMenu}>Work</a><a href="#approach" onClick={closeMenu}>Approach</a><a href="#services" onClick={closeMenu}>Services</a><a href="#about" onClick={closeMenu}>About</a><a className="nav-cta" href="#contact" onClick={closeMenu}>Start a project <Arrow /></a></nav><button className={`menu ${menuOpen ? "open" : ""}`} onClick={() => setMenuOpen(!menuOpen)} aria-label="Toggle menu" aria-expanded={menuOpen}><i /><i /></button></header>
    ...
    <section className="hero" id="top">
      ...
      <div className="hero-bottom"><MagneticLink href="#contact" className="button button-light">Start a project <Arrow /></MagneticLink><a className="text-link" href="#work">Explore our work <span>↓</span></a><p>BRAND STRATEGY / VISUAL IDENTITY / DIGITAL</p></div>
