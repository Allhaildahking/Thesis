...
const services = [
  ...
  { number: '02', name: 'Visual identity', copy: 'A visual system with enough character to be remembered and enough discipline to scale.' },
  { number: '03', name: 'Digital experience', copy: 'Websites and interfaces that turn attention into trust, then trust into action.' },
  { number: '04', name: 'Social presence', copy: 'An honest audit of how your brand shows up, speaks and gets understood online.' },
];

const work = [
  { name: 'SAGENIX', type: 'Strategy / Identity', line: 'Making expertise look as serious as it is.', mark: 'SG' },
  { name: 'KORA', type: 'Brand / Digital', line: 'Giving a good idea a sharper point of view.', mark: 'KO' },
];
  ...
  return (
    <main className="experience" style={{ '--mx': `${mouse.x}%`, '--my': `${mouse.y}%` } as React.CSSProperties}>
      ...
      <section className="hero" id="top">
        ...
        <div className="hero-stamp" aria-hidden="true"><Image src={thesisLogo} alt="" sizes="72px" /></div>
      </section>
      ...
      <section className="challenge" id="question">
        ...
        <div className="challenge-content">
          ...
          {answer !== null && <div className="answer" role="status">{answer ? 'Good. Now make sure the rest of the experience earns that memory.' : 'That is exactly the kind of problem Thesis exists to solve.'}</div>}
        </div>
      </section>
      ...
      <section className="services" id="services">
        ...
        <div className="services-grid">
          <div><p className="kicker">Not just pretty.</p><h2>We make the<br /><em>idea</em> impossible<br />to miss.</h2></div>
          <div className="service-list">
            {services.map((service, index) => <button key={service.number} className={activeService === index ? 'service active' : 'service'} onMouseEnter={() => setActiveService(index)} onFocus={() => setActiveService(index)} onClick={() => setActiveService(index)}><span>{service.number}</span><strong>{service.name}</strong><i>↗</i></button>)}
            <div className="service-detail" key={services[activeService].number}><span>{services[activeService].number}</span><p>{services[activeService].copy}</p></div>
          </div>
        </div>
      </section>
      ...
      <section className="work" id="work">
        ...
        <div className="work-heading"><p className="kicker">Proof, not promises.</p><h2>Things we&apos;ve<br /><em>made matter.</em></h2></div>
        <div className="work-grid">
          {work.map((project, index) => <a className={`project project-${index + 1}`} href="#start" key={project.name}><div className="project-art"><span>{project.mark}</span><small>THESIS / {String(index + 1).padStart(2, '0')}</small></div><div className="project-meta"><div><strong>{project.name}</strong><span>{project.type}</span></div><b>↗</b></div><p>{project.line}</p></a>)}
        </div>
      </section>

      <section className="manifesto"><div className="manifesto-line">A brand is not a logo.</div><div className="manifesto-line indent">It is the feeling people leave with.</div><div className="manifesto-line">We build <em>that.</em></div></section>
